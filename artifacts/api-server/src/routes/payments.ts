...
router.get("/payments", requireAuth, async (req, res) => {
  ...
  let payments = await db.query.paymentsTable.findMany({ orderBy: (p, { desc }) => [desc(p.createdAt)] });

  if (invoiceId) payments = payments.filter(p => p.invoiceId === Number(invoiceId));
  ...
  if (clientId) {
    ...
    result = result.filter(p => invMap[p.invoiceId]?.clientId === cid);
  }

  res.json(result);
});

router.post("/payments", requireAuth, requireRole("operations"), async (req, res) => {
